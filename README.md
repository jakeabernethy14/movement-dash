...
- **Progress tab, simplified**: weight graph, latest measurements, and strength/PB progression for clients; trainers get the same plus workout adherence % and nutrition logging %. All key metrics (sleep, energy, adherence, logging rate) are color-coded green/orange/red. Progress photos were removed after feedback — the underlying table and private storage bucket from migration_006 are still there but unused, safe to ignore or drop later.
- **Program assignment now supports ad-hoc sessions**: a PT can either assign a day from a saved plan, or create a one-off custom session on the spot (title + description), both placed by clicking a date on the client's calendar.
- **Sessions tab redesigned as a calendar**: per-client session creation now happens from the Program tab (plans) or Sessions & Classes (calls) — this tab is purely for reviewing what's scheduled and whether the client responded, with days highlighted green once they log a result and a hover "View" button for details.
- **Sessions & Classes** (PT global page) now only shows personal schedule items and client calls — a client's day-by-day workout plan lives on their own Program/Sessions tabs instead of cluttering this list.
- **Token management**: PT Admin and Owner token lists now color-code usage (green = still available, red = fully redeemed), show who redeemed each token, and Owner can now delete trainer-invite tokens (previously only PT-issued client tokens were deletable).
- **News & Notices** now shows the poster's role in bright green next to their name.
...
8. Then run `supabase/migration_007.sql` — tracks who redeemed each registration token, so the PT Admin / Owner token lists can show it.
9. (Optional but recommended) Turn on **Realtime** for the `messages` table so chats update live without a refresh: **Database → Replication** in the Supabase dashboard, find `messages`, and toggle it on. Without this, messages still send/receive fine — the other person just needs to reopen or revisit the Messages page to see new ones instead of seeing them appear instantly.
10. Go to **Authentication → Sign In / Providers → Email** and decide whether "Confirm email" is on or off
   (the in-app Settings toggle is a preference flag for your own reference — flip the real switch here too).
11. Grab your **Project URL** and **Publishable key** from **Project Settings → API Keys**
...
12. Also grab your **service_role** key (same page, "Reveal" next to `service_role`) — this is needed
